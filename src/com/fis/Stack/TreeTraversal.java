...
class BinaryTree {
	...
	public void inOrderUsingStack() {
		...
		/*
		 * 1) Create an empty stack S. 2) Initialize current node as root 3)
		 * Push the current node to S and set current = current->left until
		 * current is NULL 4) If current is NULL and stack is not empty then a)
		 * Pop the top item from stack. b) Print the popped item, set current =
		 * popped_item->right c) Go to step 3. 5) If current is NULL and stack
...
public class TreeTraversal {
	...
	public static void main(String[] args) {
		...
		//tree.root = constructTreeFromInorderAndPreorder(inorder,preOrder,0,inorder.length-1);
	}
